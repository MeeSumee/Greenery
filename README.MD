...
- [Greenery](https://github.com/MeeSumee/Greenery?tab=readme-ov-file#greenery): x86-based server with multiple services.
- [Verdure](https://github.com/MeeSumee/Greenery?tab=readme-ov-file#verdure): Arm-based server with multiple services.
...
- [Quartz](https://github.com/MeeSumee/Greenery?tab=readme-ov-file#quartz): Gaming and Workstation PC.

## Installation
You may install as a flake to access my programs and modules using:
```nix
    # MeeSumee config
    greenery = {
      url = "github:MeeSumee/Greenery/stable"; # Use stable config
      inputs = {
        nixpkgs.follows = "nixpkgs"; # To prevent different nixpkgs versions for your system
        asusnumpad.follows = ""; # Example of disabling asusnumpad flake if you don't want to use it
      };
    };
```
The downside is you pull my entire repository to use as your flake (roughly 53MB).

You may also run directly as a VM to test the system out:
```bash
git clone https://github.com/MeeSumee/Greenery

cd Greenery

# I highly recommend adding more RAM for GUI based configs (at least 5GB)

nix run .#nixosConfigurations.$HOSTNAME.config.system.build.vm
```

I also documented how to install from scratch on my [wiki](https://github.com/MeeSumee/Greenery/wiki/Installation)
...
Hosts Jellyfin, Immich, FileBrowser, Suwayomi, MotionEye & Memos.

Reverse proxied using caddy-tailscale plugin.

## Verdure

Raspberry Pi 4 Model B with 8GB of RAM.

Hosts Anki Sync, Davis, 2fauth, VaultWarden, & Home Assistant.
...
Custom-built gaming pc that I mainly use at home.

<details>
<summary>Quartz PC Specifications</summary>
<br>
I know its impossible to build a PC now. This looks more like flexing rather than useful information.
...
- [Sam-Toki-Mouse-Cursors](https://github.com/SamToki/Sam-Toki-Mouse-Cursors)
- [User & GitHub pfp](https://danbooru.donmai.us/posts/9259057?q=vivian_banshee+pyogo)
